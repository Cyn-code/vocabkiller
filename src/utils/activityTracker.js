...
class ActivityTracker {
  constructor(onAdPromptTrigger) {
    // Version check: Clear localStorage if from old version
    const APP_VERSION = '2025-08-30-timer-fix';
    const currentVersion = localStorage.getItem('vocabkiller_app_version');
    
    if (currentVersion !== APP_VERSION) {
      console.log('🔄 App version changed, clearing all localStorage');
      localStorage.removeItem('vocabkiller_session_state');
      localStorage.removeItem('vocabkiller_timer_position');
      localStorage.setItem('vocabkiller_app_version', APP_VERSION);
    }
  ...
  resetAdTimer() {
    ...
    this.adWatched = false; // Reset to false to start fresh 15-minute cycle
    this.sessionStartTime = Date.now();
  ...
  getStats() {
    ...
    // Calculate time until next ad (always 15 minutes from session start)
    const timeUntilAd = Math.max(0, this.adPromptInterval - totalElapsedTime);
    
    const stats = {
      sessionTime: totalElapsedTime,
      formattedSessionTime: this.formatTime(totalElapsedTime),
      adWatched: this.adWatched,
      timeUntilAd: timeUntilAd,
      formattedTimeUntilAd: this.formatTime(timeUntilAd)
    };
