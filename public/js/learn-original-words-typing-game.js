...
class VocabKillerTypingGame {
    constructor() {
        ...
        if (typeof SoundManager !== 'undefined') {
            ...
            console.log('Learn Original Words: Sound manager initialized');
        } else {
            console.warn('Learn Original Words: SoundManager not available');
        }
        
        // Initialize translation manager
        if (typeof SubpageTranslationManager !== 'undefined') {
            this.translationManager = new SubpageTranslationManager();
            console.log('Learn Original Words: Translation manager initialized');
        } else {
            console.warn('Learn Original Words: SubpageTranslationManager not available');
        }
        ...
        // Load sound settings after a short delay to ensure DOM is ready
        setTimeout(() => {
    ...
    setupEventListeners() {
        ...
        if (wordDisplayArea) {
            wordDisplayArea.addEventListener('click', (e) => {
                ...
                if (this.answerInputMode === 'keyboard') {
                    ...
                    if (character) {
                        this.selectKeyboardCharacter(parseInt(character.dataset.charIndex, 10));
                    }
        ...
        if (typingInputArea) {
            typingInputArea.addEventListener('click', (e) => {
    ...
    handleKeyboardTyping(e) {
        ...
        const isAnswerKey = e.key.length === 1 || e.key === 'Backspace' || e.key === 'Delete' || e.key === 'Enter';
        const isAnswerEditingKey = isAnswerKey ||
            ['Delete', 'ArrowLeft', 'ArrowRight', 'Home', 'End'].includes(e.key);
        const isModifiedPencilEdit = this.answerInputMode !== 'keyboard' &&
            e.target?.id === 'typingInput' &&
            (e.metaKey || e.ctrlKey || e.altKey) &&
            isAnswerEditingKey;
        if (isModifiedPencilEdit) {
            e.preventDefault();
            this.pulseActiveInputMode();
            return;
        }
        ...
        if (this.answerInputMode !== 'keyboard' && isAnswerEditingKey) {
            e.preventDefault();
            this.pulseActiveInputMode();
            return;
        }
        ...
        if (e.key.length === 1) {
            ...
            e.preventDefault();
            this.selectKeyboardCharacter(this.currentCharIndex - 1);
        } else if (e.key === 'ArrowRight') {
            e.preventDefault();
            this.selectKeyboardCharacter(this.currentCharIndex + 1);
        } else if (e.key === 'Home') {
            e.preventDefault();
            this.selectKeyboardCharacter(0);
        } else if (e.key === 'End') {
            e.preventDefault();
            this.selectKeyboardCharacter(this.wordCharacters.length - 1);
        } else if (e.key === 'Enter') {
            e.preventDefault();
    ...
    shouldIgnoreKeyboardEvent(e) {
        ...
        if (target.tagName === 'INPUT' && !['typingInput', 'keyboardCaptureInput'].includes(target.id)) return true;

        return false;
    }

    resetAnswerDrafts() {
        const length = this.wordCharacters.length || (this.words[this.currentIndex] || '').length;
        this.keyboardDraft = this.emptyKeyboardSlot.repeat(length);
        this.pencilDraft = '';
        this.typedInput = '';
        this.currentCharIndex = 0;
        this.typedCharacters = [];
    ...
    getActiveDraft() {
        return this.answerInputMode === 'keyboard' ? this.keyboardDraft : this.pencilDraft;
    }

    escapeCharacterHtml(value) {
        return String(value)
            .replace(/&/g, '&amp;')
            .replace(/</g, '&lt;')
            .replace(/>/g, '&gt;')
    ...
    selectKeyboardCharacter(index) {
        if (!this.wordCharacters.length) return;
        this.currentCharIndex = Math.max(0, Math.min(index, this.wordCharacters.length - 1));
        this.updateWordDisplay();
        this.focusKeyboardCapture();
    }

    insertKeyboardText(text) {
        if (this.answerInputMode !== 'keyboard' || this.isCompletingWord) return;
        for (const character of String(text || '')) {
            if (!this.wordCharacters.length) break;
            const slots = this.keyboardDraft.padEnd(this.wordCharacters.length, this.emptyKeyboardSlot).split('');
            slots[this.currentCharIndex] = character;
            this.keyboardDraft = slots.join('');
            this.typedInput = this.keyboardDraft;
            if (this.currentCharIndex < this.wordCharacters.length - 1) this.currentCharIndex += 1;
        }
        this.startGame();
        this.updateWordDisplay();
        if (this.soundManager?.enabled) this.soundManager.playTypingSound();
        if (this.isCurrentAnswerCorrect()) this.completeWord();
    }

    handleKeyboardBackspace() {
        if (!this.wordCharacters.length || this.isCompletingWord || this.currentCharIndex === 0) return;
        const targetIndex = this.currentCharIndex - 1;
        const slots = this.keyboardDraft.padEnd(this.wordCharacters.length, this.emptyKeyboardSlot).split('');
        slots[targetIndex] = this.emptyKeyboardSlot;
        this.keyboardDraft = slots.join('');
        this.typedInput = this.keyboardDraft;
    ...
    handleKeyboardDelete() {
        if (!this.wordCharacters.length || this.isCompletingWord) return;
        const slots = this.keyboardDraft.padEnd(this.wordCharacters.length, this.emptyKeyboardSlot).split('');
        slots[this.currentCharIndex] = this.emptyKeyboardSlot;
        this.keyboardDraft = slots.join('');
        this.typedInput = this.keyboardDraft;
        this.updateWordDisplay();
    }
    ...
    applyAnswerInputMode(shouldFocus = false) {
        ...
        wordDisplayArea?.classList.toggle('input-mode-active', isKeyboardMode);
    ...
    pulseActiveInputMode() {
        ...
        const activeArea = this.answerInputMode === 'keyboard'
            ? document.querySelector('.word-display-area')
    ...
    focusKeyboardCapture() {
        const input = document.getElementById('keyboardCaptureInput');
        if (this.answerInputMode === 'keyboard' && input && !input.disabled) {
            input.focus({ preventScroll: true });
        }
    }
    ...
    handleInput(value, source = 'pencil') {
        ...
        if (this.soundManager && this.soundManager.enabled) {
            console.log('Learn Original Words: Playing typing sound');
            this.soundManager.playTypingSound();
        } else {
            console.log('Learn Original Words: Sound manager not available or disabled. soundManager:', !!this.soundManager, 'enabled:', this.soundManager?.enabled);
        }
    ...
    isCurrentAnswerCorrect() {
        ...
        const answer = this.getActiveDraft();
        return !answer.includes(this.emptyKeyboardSlot) &&
            answer.normalize().toLocaleLowerCase() ===
    ...
    renderCharacterDisplay() {
        ...
        return this.wordCharacters.map((expectedChar, index) => {
            const typedChar = draft[index];
            const hasTypedChar = Boolean(typedChar && typedChar !== this.emptyKeyboardSlot);
            const isCorrect = hasTypedChar && typedChar.toLowerCase() === expectedChar.toLowerCase();
            const classes = ['word-character'];
            classes.push(hasTypedChar ? (isCorrect ? 'character-correct' : 'character-wrong') : 'character-pending');
            if (this.answerInputMode === 'keyboard' && index === this.currentCharIndex) classes.push('character-current');
            const displayChar = hasTypedChar ? typedChar : (this.dictationMode ? '_' : expectedChar);
            return `<span class="${classes.join(' ')}" data-word-index="0" data-char-index="${index}">${this.escapeCharacterHtml(displayChar)}</span>`;
        }).join(this.dictationMode ? ' ' : '');
    }

    updateWordDisplay() {
        const wordTextElement = document.querySelector('#currentWord .word-text');
        if (wordTextElement) wordTextElement.innerHTML = this.renderCharacterDisplay();
    }
    ...
    displayCurrentWord() {
        ...
        if (this.currentIndex < this.words.length) {
            ...
            this.isTypingCorrect = true;
            if (this.keyboardDraft.length !== this.wordCharacters.length) {
            ...
            const characterDisplay = this.renderCharacterDisplay();
    ...
    clearInput() {
        ...
        if (wordDisplayArea) {
            wordDisplayArea.classList.remove('correct', 'incorrect');
    ...
    showErrorFeedback() {
        const input = document.getElementById('typingInput');
        const wordDisplayArea = document.querySelector('.word-display-area');
        const feedbackTarget = this.answerInputMode === 'pencil' ? input : wordDisplayArea;
    ...
    showCompletionFeedback() {
        ...
        const wordDisplayArea = document.querySelector('.word-display-area');
        const feedbackTarget = this.answerInputMode === 'pencil' ? input : wordDisplayArea;
        feedbackTarget?.classList.add('correct');
    }
    ...
    completeGame() {
        ...
        if (completionPopup) {
            ...
            // Add keyboard listener for completion popup
            this.setupCompletionPopupKeyboard();
        }
    }
    ...
    hideCompletionPopup() {
        ...
        if (completionPopup) {
            completionPopup.classList.remove('show');
            // Remove keyboard listener for completion popup
            this.removeCompletionPopupKeyboard();
        }
    }

    setupCompletionPopupKeyboard() {
        // Add keyboard listener for completion popup
        this.completionKeyboardHandler = (e) => {
            if (e.key === 'Enter') {
                e.preventDefault();
                this.restartGame();
            } else if (e.key === 'Escape') {
    ...
    toggleSoundDropdown() {
        ...
        loadSoundSettings() {
        console.log('Learn Original Words: Loading sound settings');
        if (!this.soundManager) {
            console.warn('Learn Original Words: No sound manager available');
            return;
        }

        const settings = this.soundManager.getSettings();
        console.log('Learn Original Words: Sound settings:', settings);

        // Update checkbox
        const enableCheckbox = document.getElementById('enableTypingSound');
        if (enableCheckbox) {
            enableCheckbox.checked = settings.enabled;
            console.log('Learn Original Words: Updated checkbox to:', settings.enabled);
        } else {
            console.warn('Learn Original Words: Enable checkbox not found');
        }
    ...
    previewSound() {
        console.log('Learn Original Words: Preview sound called');
        if (this.soundManager) {
            console.log('Learn Original Words: Playing preview sound');
            this.soundManager.playTypingSound();
        } else {
            console.warn('Learn Original Words: No sound manager for preview');
        }
    }

    resetSoundSettings() {
        console.log('Learn Original Words: Resetting sound settings');
        if (this.soundManager) {
            this.soundManager.resetToDefaults();
            this.loadSoundSettings();
            console.log('Learn Original Words: Sound settings reset');
        } else {
            console.warn('Learn Original Words: No sound manager to reset');
        }
    }
    ...
    setupSoundEventListeners() {
        ...
        if (enableCheckbox) {
            enableCheckbox.addEventListener('change', (e) => {
                console.log('Learn Original Words: Checkbox changed to:', e.target.checked);
                if (this.soundManager) {
                    this.soundManager.setEnabled(e.target.checked);
                } else {
                    console.warn('Learn Original Words: No sound manager for checkbox change');
                }
            });
        }
